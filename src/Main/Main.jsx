...
const Main = () => {
  ...
  useEffect(() => {fetchSearch()},[page])
	...
	return (
      ...
      <button onClick={() => setPage(page - 1)} className='bg-purple-700 px-10 py-2 rounded-lg'>DOWN</button>
      <button onClick={() => setPage(page + 1)} className='bg-purple-700 px-12 py-2 rounded-lg'>UP</button>
      </div>
				<select className='bg-purple-700 px-4 py-2 rounded-lg'>
					<option>Soul</option>
				</select>
			</div>
      {loading ? <div className='flex justify-center items-center m-10'><BarLoader size={10} color='#a854f7'/></div> : <div className='flex justify-around items-center flex-wrap  p-4'>
