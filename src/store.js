...
export const useFetchData = create((set,get) => ({
  ...
  randomTopCharts:Math.floor(Math.random() * (10 - 1 + 1)) + 1,
	...
	fetchTopCharts: async () => {
		try {
			...
			fetch(
				`https://genius-song-lyrics1.p.rapidapi.com/chart/songs/?time_period=month&per_page=12&page=${get().random}`,
				options,
			)
				.then(response => response.json())
    ...
    try {
      ...
      fetch(`https://genius-song-lyrics1.p.rapidapi.com/song/details/?id=${get().trackId}`, options)
        .then(response => response.json())
  ...
  fetchSearch: async () => {
		try {
			...
			fetch(
				`https://genius-song-lyrics1.p.rapidapi.com/search/?q=${get().search}&per_page=9&&page=${get().page}`,
				options,
			)
				.then(response => response.json())
				.then(response => set({ albums:response, error: null, loading: false }))
