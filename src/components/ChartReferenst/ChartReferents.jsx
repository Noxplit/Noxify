...
const ChartReferents = () => {
  ...
  return (
    ...
    <img className='w-[100px] h-[100px] rounded-sm object-cover' src={item?.annotatable?.image_url} alt="" />
    <div className='text-xs dark:text-white text-gray-700'>{item?.annotatable?.title.slice(0,10)}</div>
  </div></Link> )}
